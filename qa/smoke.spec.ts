...
test('renders the footage-in episodes-out hero and the bottom funnel', async ({ page }) => {
  ...
  await expect(editorImage).toHaveAttribute('data-deferred-src', /sizzle-editor-5/)
  await editorImage.scrollIntoViewIfNeeded()
  await expect(editorImage).toHaveAttribute('src', /sizzle-editor-5/)
...
test('publishes all serving MCP profiles and individual tool pages', async ({ page, request }) => {
  ...
  expect(snapshot.profiles.app.descriptors).toHaveLength(112)
  ...
  expect(new Set(names).size).toBe(112)
